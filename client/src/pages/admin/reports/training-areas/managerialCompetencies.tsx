...
const ManagerialCompetencies = () => {
  ...
  const fetchReportData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${baseUrl}/reports/training-area/5`); // Managerial Competencies training area ID
      if (!response.ok) {
  ...
  const handleExportCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", "managerial_competencies_report.csv");
