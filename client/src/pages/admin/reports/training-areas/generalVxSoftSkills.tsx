...
const GeneralVxSoftSkills = () => {
  ...
  const fetchReportData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${baseUrl}/reports/training-area/4`); // General VX Soft Skills training area ID
      if (!response.ok) {
  ...
  const handleExportCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", "general_vx_soft_skills_report.csv");
